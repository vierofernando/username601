...
class games(commands.Cog):
    ...
    async def gdsearch(self, ctx, *args):
        if len(list(args))==0:
            await ctx.send(emote(self.client, 'error')+' | Please input a query!')
        else:
            try:
                ...
                for i in range(0, len(data)):
                    ...
                    count += 1
                embedy = discord.Embed(title='Geometry Dash Level searches for "'+str(' '.join(list(args)))+'":', description=levels, colour=get_embed_color(discord))
                await ctx.send(embed=embedy)
            except:
                await ctx.send(emote(self.client, 'error') + ' | Error: Not Found. :four::zero::four:')

    @command()
    @cooldown(3)
    async def gdprofile(self, ctx, *args):
        if len(list(args))==0: await ctx.send(emote(self.client, 'error')+' | Gimme some ARGS!')
        else:
            try:
                ...
                await ctx.send(emote(self.client, 'error')+' | Error, user not found.')
    
    @command()
    @cooldown(3)
    async def gdlogo(self, ctx, *args):
        if len(list(args))==0:
            await ctx.send(emote(self.client, 'error')+' | Please input a text!')
        else:
    ...
    async def gdbox(self, ctx, *args):
        if len(list(args))==0: await ctx.send(emote(self.client, 'error')+' | Please input a text!')
        else:
            async with ctx.channel.typing():
                text, av = urlify(str(' '.join(list(args)))), ctx.author.avatar_url
                if len(text)>100: await ctx.send(emote(self.client, 'error') +' | the text is too long!')
                else:
    ...
    async def rps(self, ctx):
        main = await ctx.send(embed=discord.Embed(title='Rock Paper Scissors game.', description='Click the reaction below. And game will begin.', colour=get_embed_color(discord)))
        exp = ['✊', '🖐️', '✌']
        ...
        colors = [get_embed_color(discord), discord.Colour.orange(), get_embed_color(discord)]
        if beginGame:
            embed = discord.Embed(
            ...
            if msgId==1 and Economy.get(ctx.author.id)!=None:
    ...
    async def bomb(self, ctx):
        def embedType(a):
            ...
            elif a==2: return discord.Embed(title='The bomb defused!', description='Congratulations! :grinning:', colour=get_embed_color(discord))
        embed = discord.Embed(title='DEFUSE THE BOMB!', description='**Cut the correct wire!\nThe bomb will explode in 15 seconds!**', colour=get_embed_color(discord))
        main = await ctx.send(embed=embed)
        buttons = ['🔴', '🟡', '🔵', '🟢']
    ...
    async def guessnum(self, ctx):
        ...
        while gameplay:
            ...
            try:
                ...
                await ctx.send(emote(self.client, 'error') +' | You did not respond for the next 20 seconds!\nGame ended.')
                gameplay = False
                break
