...
class economy(commands.Cog):
    ...
    async def reset(self, ctx):
        ...
        if data is None: raise self.client.utils.send_error_message("Doesn't have a profile yet. Try `1new` to have a profile.")
        else:
    ...
    async def transfer(self, ctx, *args):
        if len(args)==0 or len(ctx.message.mentions)==0: raise self.client.utils.send_error_message("Please send a mention and an amount to transfer.")
            ...
            if amount is None: raise self.client.utils.send_error_message("Please give a valid amount.")
    ...
    async def rob(self, ctx, *args):
        if len(ctx.message.mentions)==0 or len(args)==0:
            ...
            if ctx.message.mentions[0].id==ctx.author.id:
                ...
                if amount2rob is None: raise self.client.utils.send_error_message('How many bobux shall be robbed?')
