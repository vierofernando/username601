...
class economy(commands.Cog):
    ...
    async def beg(self, ctx):
        ...
        if self.client.economy.get(ctx.author.id)==None: raise noProfile()
        if c==1:
            award = random.randint(100, 500)
            self.client.economy.addbal(ctx.author.id, award)
    ...
    async def fish(self, ctx):
        ...
        if res['catched']:
            ...
            self.client.economy.addbal(ctx.author.id, awards)
    ...
    async def buylist(self, ctx):
        ...
        if self.client.economy.get(ctx.author.id)==None: raise noProfile()
        try:
            data = self.client.economy.getBuyList(source)
    ...
    async def addproduct(self, ctx, *args):
        ...
        try:
            ...
            a = self.client.shop.add_value(productName, price, ctx.guild)
    ...
    async def buy(self, ctx, *args):
        ...
        if self.client.economy.get(ctx.author.id)==None: raise noProfile()
        try:
            data = self.client.shop.buy(' '.join(list(args)), ctx.author)
            assert data['error']==False, data['ctx']
            return await ctx.send('{} | {}!'.format(self.client.utils.emote(self.client, 'success'), data['ctx']))
        except Exception as e:
            await ctx.send('{} | {}!'.format(self.client.utils.emote(self.client, 'error'), str(e)))
    ...
    async def shop(self, ctx):
        try:
            data = self.client.shop.get_shop(ctx.guild)
            assert data['error']==False, data['ctx']
    ...
    async def reset(self, ctx):
        ...
        data = self.client.economy.get(ctx.author.id)
        if data==None: raise noProfile()
            ...
            if 'y' in str(waiting.content).lower():
                self.client.economy.delete_data(ctx.author.id)
    ...
    async def work(self, ctx):
        ...
        if data==None: raise noProfile()
            ...
            new_data = self.client.economy.addbal(ctx.author.id, int(reward))
    ...
    async def daily(self, ctx, *args):
        ...
        if self.client.economy.get(ctx.author.id)==None: raise noProfile()
            ...
            obj = self.client.economy.can_vote(ctx.author.id)
            if obj['bool']:
    ...
    async def transfer(self, ctx, *args):
        if len(list(args))==0 or len(ctx.message.mentions)==0: await ctx.send(self.client.utils.emote(self.client, 'error')+' | gimme some tag and some amount.')
            ...
            if amount==None: await wait.edit(content=self.client.utils.emote(self.client, 'error')+' | Give me some valid amount!')
            ...
            elif self.client.economy.get(ctx.author.id)==None or self.client.economy.get(ctx.message.mentions[0].id)==None:
                ...
                self.client.economy.addbal(ctx.message.mentions[0].id, amount)
                self.client.economy.delbal(ctx.author.id, amount) # EFFICIENT CODE LMFAO
        ...
        if len(ctx.message.mentions)==0 or len(list(args))==0:
            ...
            if ctx.message.mentions[0].id==ctx.author.id:
                ...
                if amount2rob==None: await ctx.send(self.client.utils.emote(self.client, 'error')+' | How many bobux shall be robbed?')
                    ...
                    victim, stealer = self.client.economy.get(ctx.message.mentions[0].id), self.client.economy.get(ctx.author.id)
                    if victim==None or stealer==None:
                        ...
                        if not str(data['amount']).replace('-', '').isnumeric():
                            ...
                            else: robamount = -self.client.economy.get(ctx.author.id)['bal']
                        else: robamount = data['amount']
                        if robamount > 0:
                            self.client.economy.addbal(ctx.author.id, robamount) ; self.client.economy.delbal(ctx.message.mentions[0].id, robamount)
                            statement = f'You stole {str(robamount)} in total.'
                        elif robamount==0:
                            statement = f'You left empty-handed.'
                        else:
                            self.client.economy.delbal(ctx.author.id, robamount*-1) ; self.client.economy.addbal(ctx.message.mentions[0].id, robamount*-1)
    ...
    async def deposit(self, ctx, *args):
        ...
        if data==None: raise noProfile()
        if list(args)[0].lower()=='all':
            self.client.economy.deposit(ctx.author.id, data['bal'])
        ...
        try:
            ...
            self.client.economy.deposit(ctx.author.id, num)
    ...
    async def withdraw(self, ctx, *args):
        ...
        if data==None: raise noProfile()
        if list(args)[0].lower()=='all':
            self.client.economy.withdraw(ctx.author.id, data['bankbal'])
        ...
        try:
            ...
            self.client.economy.withdraw(ctx.author.id, num)
    ...
    async def leaderboard(self, ctx):
        wait = await ctx.send(self.client.utils.emote(self.client, 'loading')+' | Please wait...')
        data = self.client.economy.leaderboard(ctx.guild.members)
        if len(data)==0:
    ...
    async def setdesc(self, ctx, *args):
        if len(list(args))==0:
            ...
            if len(list(args))>120:
                ...
                wait = await ctx.send(self.client.utils.emote(self.client, 'loading')+' | Please wait...')
                if self.client.economy.get(ctx.author.id)==None:
                    raise noProfile()
                else:
                    data = self.client.economy.setdesc(ctx.author.id, str(' '.join(list(args))))
                    if data=='error':
    ...
    async def bal(self, ctx, *args):
        ...
        if self.client.economy.get(src.id)==None:
            raise noProfile()
        else:
            data = self.client.economy.getProfile(src.id, [i.id for i in ctx.guild.members if not i.bot])
    ...
    async def new(self, ctx):
        ...
        if data!=None:
            ...
            data = self.client.economy.new(ctx.author.id)
            if data!='done':
