...
class encoding(commands.Cog):
    ...
    async def morse(self, ctx, *args):
        if len(args)==0: raise ctx.bot.utils.send_error_message('no arguments? Really?')
        elif len(' '.join(args)) > 100:
            raise ctx.bot.utils.send_error_message('too long....')
        else:
    ...
    async def barcode(self, ctx, *args):
        if len(args)==0:
            ...
            raise ctx.bot.utils.send_error_message('too longggggggggg')
        else:
            async with ctx.channel.typing():
        ...
        if len(args)<2:
            ...
            for i in args:
                if i.isnumeric():
                    offset = int(i); break
            if offset is None:
                raise ctx.bot.utils.send_error_message('No offset?')
            else:
