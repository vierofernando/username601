...
class encoding(commands.Cog):
    ...
    async def morse(self, ctx, *args):
        ...
        if res is None: raise ctx.bot.util.BasicCommandException("The API is temporarily down. Please try again later.")
        ...
        if len(args)<2:
            ...
            for i in args:
                if i.isnumeric():
                    offset = int(i); break
            if offset is None:
                raise ctx.bot.util.BasicCommandException('No offset?')
            else:
    ...
    async def atbash(self, ctx, *args):
        ctx.bot.Parser.require_args(ctx, args)
        return await ctx.send(ctx.bot.util.atbash(' '.join(args)))

    @command()
    @cooldown(2)
    async def reverse(self, ctx, *args):
        ctx.bot.Parser.require_args(ctx, args)
        return await ctx.send(str(' '.join(args))[::-1], allowed_mentions=discord.AllowedMentions(everyone=False, users=False, roles=False))
    
    @command('b64')
    @cooldown(2)
    async def base64(self, ctx, *args):
        ctx.bot.Parser.require_args(ctx, args)
        return await ctx.send(ctx.bot.util.base64(' '.join(args)))
